...
def incident_diffuse(tilt, light_form):
    ...
    i=0
    # if tilt is equal to zero, no ground-reflected light should reach a solar cell
...
def sim_refracted_vect(wavelength, angle, theta_df, phi_df):
    """ Refracted Vector from COMSOL"""
    p = phi_df
    d = theta_df
    ...
    phi = p.loc[wavelength,angle](rand)
    theta = d.loc[wavelength,angle](rand)
    refr_vect = sph2cart(theta, phi)
    refr_vect = rm.z(math.pi/2, refr_vect)
    #testboi = cart2sph(refr_vect)
    #print(testboi)
    return refr_vect
...
def sim_refraction_or_reflection(forward_rho, backward_rho, vect, tilt, refracted_vect, indexmatch, n, p_o, p_i, bdy_center, surface_type = 'specular'):
    """Determine if a bundle is refracted or reflected at an interface.
    
    Parameters
    ----------
    forward_rho : float
        Reflectivity value when bundle is attempting to enter LSC through nanostructure
    backward_rho : float
        Reflectivity value when bundle is attempting to exit LSC through nanostructure
    vect : NumPy array
        Current bundle trajectory in local coordinates
    tilt : float
        Boundary angle relative to the xy plane
    refracted_vect : NumPy array, string
        Refracted vector - bundle trajectory after refraction
    indexmatch : int
        If indexmatch is 1, bundle remains in the same volume or is lost
        If indexmatch is 0, bundle has the opportunity to move to an
        adjacent volume.
    ...
    # If bundle is leaving LSC, use reflectivity associated with backward direction 
    if dist_ext_point_p_o < dist_ext_point_p_i:
        rho = backward_rho
    # Otherwise it is entering LSC, use reflectivity associated with forward direction
    else:
        rho = forward_rho
    #print(rho)   
    if random.uniform(0, 1) < rho:
...
def refracted_vect_flip(n, bdy_center, n_max, p_o, p_i, vect):
    ...
    if dist_ext_point_p_o < dist_ext_point_p_i:
        ...
        #print("less than", refract_vect)
    else:
        refract_vect = refracted_vect(1.0, n_max, vect)
        #print("else", refract_vect)
    #print(refract_vect)
    return refract_vect

def sim_refracted_vect_flip(n, bdy_center, p_o, p_i, wavelength, angle, theta_df, phi_df):
    """Determines if a bundle is attempting to leave an LSC or if it is
    attempting to enter the LSC. Then, finds the refracted vector.
    ...
    p_i : NumPy array
        ...
        it is not interrupted on the way (i.e. absorbed by the matrix or bundle
                                          intersects a particle).
    wavelength : float
        Wavelength of the bundle
    angle : float
        Polar incident angle at the boundary in local coordinates
    theta_eq : DataFrame
        Table of regression equations for calculating bundle trajectory after bundle enters the LSC through nanostructure
    phi_eq : DataFrame
        Table of regression equations for calculating bundle trajectory after bundle enters the LSC through nanostructure
    ...
    refract_vect = 0
    if dist_ext_point_p_o < dist_ext_point_p_i:
        # If bundle is leaving the LSC through nanostructure, set refracted vector as [0,0,1]
        refract_vect = -n
        #print("less than", refract_vect)
    else:
        # If bundle is entering the LSC through nanostructure, calculate resulting trajectory using regression equations
        refract_vect = sim_refracted_vect(wavelength, angle, theta_df, phi_df)
        #print("else", refract_vect)
    #print(refract_vect)
    return refract_vect
